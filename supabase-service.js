...
const SupabaseService = {
  ...
  /**
   * Save or update a review draft (auto-save)
   */
  async saveReviewDraft(championId, panelId, indicatorId, draftData) {
    try {
      const necessary = draftData.necessary || null;
      const rating = draftData.rating || null;
      const comments = draftData.comments || null;
      const progressPercentage = draftData.progressPercentage || 0;

      const { data, error } = await supabaseClient.rpc('save_review_draft', {
        p_champion_id: championId,
        p_panel_id: panelId,
        p_indicator_id: indicatorId,
        p_necessary: necessary,
        p_rating: rating,
        p_comments: comments,
  ...
  /**
   * Get latest review draft for champion
   */
  async getLatestReviewDraft(championId) {
    try {
      const { data, error } = await supabaseClient.rpc('get_latest_review_draft', {
        p_champion_id: championId
      });
      ...
      console.error('Get latest review draft error:', error);
      return null;
    }
  },

  /**
   * Get review draft for specific indicator
   */
  async getReviewDraft(championId, indicatorId) {
    try {
      const { data, error } = await supabaseClient
        .from('review_drafts')
        .select('*')
   ...
   * Delete review draft
   */
  async deleteReviewDraft(championId, indicatorId) {
    try {
